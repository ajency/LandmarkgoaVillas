...
class WhizzyPlugin {
	...
	function enqueue_styles() {
		if ( ! wp_style_is( 'wpgrade-main-style' ) && self::$plugin_settings['disable_whizzy_style'] !== '1' ) {
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( 'thumbnails_popup', plugins_url(  'assets/js/thumbnails-popup.js',__FILE__ ), array( 'jquery' ),  $this->version, true   );

		wp_enqueue_script( $this->plugin_slug . '-plugin-script', plugins_url( 'assets/js/public.js', __FILE__ ), array( 'jquery' ), $this->version, true );
	...
	function add_action_links( $links ) {
		return array_merge( array( 'settings' => '<a href="' . admin_url( 'options-general.php?page=whizzy' ) . '">' . esc_html__( 'Settings', 'whizzy' ) . '</a>' ), $links );
	}
	...
	function hook_into_the_content( $content ) {
		...
		if ( self::$plugin_settings['disable_whizzy_style'] !== '1' && file_exists( $whizzy_path . 'assets/css/public.css' ) ) {
			ob_start();
			echo '<style>';
