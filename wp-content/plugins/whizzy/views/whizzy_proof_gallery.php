...
<?php if ( isset( $show_filters ) && $show_filters == 'yes' && $whizzy_style == 'masonry'){ ?>
    <div class="whizzy-filters">
        <button class="button is-checked" data-filter="*"><?php esc_html_e('All', 'whizzy'); ?></button>
        <button class="button" data-filter=".selected">
			<?php esc_html_e('Selected', 'whizzy'); ?>
        </button>
        <button class="button" data-filter=":not(.selected)">
			<?php esc_html_e('Rejected', 'whizzy'); ?>
        </button>
    </div>
<?php } ?>

<div id="whizzy_proof_gallery" class="gallery row gallery-columns-<?php echo esc_attr( $columns . ' ' . $whizzy_style . ' ' . $whizzy_popup ); ?>  cf  js-whizzy-gallery" data-space="<?php echo esc_attr( $whizzy_space ); ?>">
    <?php
	$idx = 1;
